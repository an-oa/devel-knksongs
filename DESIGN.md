...
- **サイドバー**：検索・絞り込み・設定
  ...
  - 配信での立場フィルタ（すべて/ホスト/ゲスト。ホストは配信での立場がゲスト以外、ゲストはゲスト行を対象にする）
  - リレー/ハモリ
  - ブックマーク導線（専用パネルを開く）
  - 設定導線（専用パネルを開く）
- **サイドバー内設定パネル**：表示設定
...
- 配信での立場：`ゲスト` は `streamRole` が `ゲスト` の行のみ、`ホスト` は `ゲスト` 以外の行を対象にする
...
- 図中の「条件変更」は、キーワード入力・日付指定・形態の絞り込み・配信での立場・リレー/ハモリONをまとめた表記。
