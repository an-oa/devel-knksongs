 ...
 * @param {string[]} args
 * @returns {{ inputPath: string, outputPath: string, sourceUrl: string }}
 */
 ...
 * @returns {string}
 */
function stringifySongs(songs) {
    return `${JSON.stringify(buildSongsJsonPayload(songs), null, 2)}\n`;
}

/**
 * CSV から曲データJSONを生成してファイルへ保存する。
 * @param {{ inputPath: string, outputPath: string, sourceUrl: string }} options
...
async function buildSongsJson(options) {
    ...
    await writeFile(resolve(options.outputPath), stringifySongs(songs), "utf8");
    return songs.length;
}
...
try {
    ...
    console.log(`Generated ${options.outputPath} (${count} songs)`);
} catch (error) {
