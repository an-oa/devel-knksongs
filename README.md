...
- 通常の曲データ表示では GitHub Pages 上の生成済みJSONを取得します。JSON生成やフォールバック時には Google(スプレッドシートのCSV取得)、YouTube表示では youtube-nocookie.com、再生制御用の YouTube Iframe API、サムネイル配信元などへの通信が発生します。
...
- YouTubeへのリンクがあります。
  ...
  - 埋め込み再生は `youtube-nocookie.com` の Privacy Enhanced Mode 埋め込みを使用します(曲名リンクは通常の `youtube.com` / `youtu.be` を開きます)。
    - 再生状態の制御には `youtube.com` から配信される YouTube Iframe API を使用します。
